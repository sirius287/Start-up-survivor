...
// Deploy allowances (tunable per event via env without a code change).
const MAX_TICKS_PER_TEAM = Math.max(1, Number.parseInt(process.env.MAX_TICKS_PER_TEAM || '10', 10) || 10);
const MAX_DEPLOYS_PER_HOUR = Math.max(1, Number.parseInt(process.env.MAX_DEPLOYS_PER_HOUR || '2', 10) || 2);
...
async function loadSnapshot(user) {
  ...
  if (user.role === 'team') { const ownId = String(user.teamId); if (states[ownId]) { const h = await hourlyDeployCount(pool, idOf(user.teamId)); states[ownId].allowance = allowanceFor(states[ownId].deployCount || 0, h.count); } }
...
/* Deploys in the trailing 60 minutes (rolling window) for the hourly cap. */
async function hourlyDeployCount(client, teamId) {
  const { rows } = await client.query("SELECT COUNT(*)::int AS n, MIN(created_at) AS oldest FROM market_history WHERE team_id=$1 AND created_at > NOW() - INTERVAL '1 hour'", [teamId]);
  return { count: rows[0]?.n || 0, oldest: rows[0]?.oldest ? new Date(rows[0].oldest).getTime() : null };
}
function allowanceFor(deployCount, hourlyCount) {
  return { ticksUsed: deployCount, ticksMax: MAX_TICKS_PER_TEAM, hourlyUsed: hourlyCount, hourlyMax: MAX_DEPLOYS_PER_HOUR };
}
...
app.get('/api/deployments', requireAuth, requireRole('judge'), async (req, res, next) => { try {
