...
app.post('/api/game/:action', requireAuth, requireRole('judge'), async (req, res, next) => { try { const actions = { start:['lobby','active'], pause:['active','paused'], resume:['paused','active'], end:['active','ended'] }; const transition = actions[req.params.action]; if (!transition) return fail(res, 400, 'INVALID_ACTION', 'Unknown game action.'); const game = await getGame(); if (game.phase !== transition[0]) return fail(res, 409, 'INVALID_GAME_TRANSITION', `Cannot ${req.params.action} the game while it is ${game.phase}.`); await pool.query('UPDATE game_state SET phase=$1,game_start_time=COALESCE(game_start_time,CASE WHEN $1=\'active\' THEN NOW() ELSE game_start_time END),updated_at=NOW() WHERE id=1', [transition[1]]); ok(res, await loadSnapshot(req.user)); } catch (e) { next(e); } });
